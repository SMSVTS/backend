...
def area_master(request):
    ...
    if request.method == "POST":
        ...
        errors = {
            'area_name': {'req_msg': 'Area is required','val_msg': '', 'type': ''},
        }
        ...
        is_exist, message = check_existing_value(area_name, "area_name", table_name)
        if is_exist:
            return JsonResponse({'status': 400, 'action': 'error_group', 'message': message,"message_type":"specific"}, safe=False) 

        query = f"""insert into area_master (data_uniq_id,area_name,created_date,modified_date,created_by,modified_by) values ('{data_uniq_id}','{area_name}','{utc_time}', '{utc_time}','{user_id}','{user_id}');"""
    ...
    #To modify the data
        ...
        is_exist, message = check_existing_value(area_name, "area_name", table_name, data_uniq_id)
        if is_exist:
            return JsonResponse({'status': 400, 'action': 'error_group', 'message': message,"message_type":"specific"}, safe=False) 

        query = f"""update area_master set area_name = '{area_name}', modified_date = '{utc_time}',modified_by = '{user_id}' where data_uniq_id = '{data_uniq_id}';"""
    ...
    #To delete the data
        ...
        data_uniq_id = base64_operation(data["data_uniq_id"],'decode')  
        query = """DELETE FROM area_master where data_uniq_id = '{data_uniq_id}';""".format(data_uniq_id=data_uniq_id)
...
def area_master_get(request):
    ...
    The `area_master_get` API is responsible for fetching data from the master database
    ...
    fetch_data_query = """ SELECT *, TO_CHAR(area_master.created_date, 'Mon DD, YYYY | HH12:MI AM') as created_f_date,(select user_name from user_master where user_master.data_uniq_id = area_master.created_by) as created_user FROM area_master WHERE 1=1  {search_join} {order_by} {limit};""".format(search_join=search_join,order_by=order_by,limit=limit_offset) 
...
@validate_access_token
@handle_exceptions
def area_master_status(request):
    data = json.loads(request.body)
    utc_time = datetime.utcnow()
    ...
    for data_uniq_id in data_uniq_id_list:
        ...
        query = """UPDATE area_master SET active_status = {active_status}, modified_date = '{modified_date}', modified_by = '{modified_by}' WHERE data_uniq_id = '{data_uniq_id}';""".format(data_uniq_id=data_uniq_id_en, active_status=active_status, modified_date=utc_time, modified_by=user_id)
...
@validate_access_token
@handle_exceptions
def area_master_delete(request):
    data = json.loads(request.body)    
    #To throw an required error message
    errors = {
        'data_ids': {'req_msg': 'ID is required','val_msg': '', 'type': ''}, 
    ...
    for data_uniq_id in data_uniq_id_list:
        ...
        query = """delete from area_master where data_uniq_id = '{data_uniq_id}';""".format(data_uniq_id=data_uniq_id_en)                
        execute = django_execute_query(query)
    success_message = "Data deleted successfully"
    error_message = "Failed to delete data"
    if execute == 0:
