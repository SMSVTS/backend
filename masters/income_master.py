...
@handle_exceptions
def income_type_master(request):
    """
    ...
    The `income_type_master` API is responsible for adding new records to the master database.
    ...
    if request.method == "POST":
        ...
        is_exist, message = check_existing_value(income_type, "income_type", table_name)
        if is_exist:
            return JsonResponse({'status': 400, 'action': 'error_group', 'message': message,"message_type":"specific"}, safe=False) 

        query = f"""insert into income_type_master (data_uniq_id,income_type,created_date,modified_date,created_by,modified_by) values ('{data_uniq_id}','{income_type}','{utc_time}', '{utc_time}','{user_id}','{user_id}');"""
    ...
    #To modify the data
        ...
        is_exist, message = check_existing_value(income_type, "income_type", table_name, data_uniq_id)
        if is_exist:
            return JsonResponse({'status': 400, 'action': 'error_group', 'message': message,"message_type":"specific"}, safe=False) 

        query = f"""update income_type_master set income_type = '{income_type}', modified_date = '{utc_time}',modified_by = '{user_id}' where data_uniq_id = '{data_uniq_id}';"""
    ...
    #To delete the data
        ...
        data_uniq_id = base64_operation(data["data_uniq_id"],'decode')  
        query = """DELETE FROM income_type_master where data_uniq_id = '{data_uniq_id}';""".format(data_uniq_id=data_uniq_id)
...
@validate_access_token
@handle_exceptions
def income_type_master_get(request):
    ...
    The `income_type_master_get` API is responsible for fetching data from the master database
    ...
    if search_input:
        ...
        search_join += " AND ({table_name}.income_type ILIKE '{inp}') ".format(inp=search_input,table_name=table_name)
    ...
    fetch_data_query = """ SELECT *, TO_CHAR(income_type_master.created_date, 'Mon DD, YYYY | HH12:MI AM') as created_f_date,(select user_name from user_master where user_master.data_uniq_id = income_type_master.created_by) as created_user FROM income_type_master WHERE 1=1  {search_join} {order_by} {limit};""".format(search_join=search_join,order_by=order_by,limit=limit_offset) 
...
@validate_access_token
@handle_exceptions
def income_type_master_status(request):
    data = json.loads(request.body)
    utc_time = datetime.utcnow()
    errors = {
    ...
    for data_uniq_id in data_uniq_id_list:
        ...
        query = """UPDATE income_type_master SET active_status = {active_status}, modified_date = '{modified_date}', modified_by = '{modified_by}' WHERE data_uniq_id = '{data_uniq_id}';""".format(data_uniq_id=data_uniq_id_en, active_status=active_status, modified_date=utc_time, modified_by=user_id)
...
@validate_access_token
@handle_exceptions
def income_type_master_delete(request):
    data = json.loads(request.body)
    errors = {
        'data_ids': {'req_msg': 'ID is required','val_msg': '', 'type': ''}, 
    }
    ...
    for data_uniq_id in data_uniq_id_list:
        ...
        query = """delete from income_type_master where data_uniq_id = '{data_uniq_id}';""".format(data_uniq_id=data_uniq_id_en)                
        execute = django_execute_query(query)
    success_message = "Data deleted successfully"
    error_message = "Failed to delete data"
    if execute == 0:
