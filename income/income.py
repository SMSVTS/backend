...
def income_create(request):
    ...
    The `income` API is responsible for adding new records to the master database.
    ...
    if income_type_id != "" and income_type_id != None:
        income_type_id = base64_operation(income_type_id,'decode') 
    ...
    #To throw an required error message
        errors = {
            'income_type': {'req_msg': 'Income Type is required','val_msg': '', 'type': ''},
            'income_date': {'req_msg': 'Income Date is required','val_msg': '', 'type': ''},
            'income_type_id': {'req_msg': 'Income Type is required','val_msg': '', 'type': ''},
        ...
        query = f"""INSERT INTO income (data_uniq_id,income_type,income_type_id,cash_type,remarks,amount,income_date,created_date,modified_date,created_by,modified_by) values ('{data_uniq_id}','{income_type}','{income_type_id}','{cash_type}','{remarks}','{amount}','{income_date}','{utc_time}', '{utc_time}','{user_id}','{user_id}');"""
        notification = f"""Dear Sir,  
        Your income has been Debited successfully.
        📅 Date: {income_date}  
        💰 Debit Amount: {amount}  
        💳 Payment Type: {cash_type}  
        """
        notification_insert = f"""
        INSERT INTO notification_table (data_uniq_id, created_by, created_date, notification,notification_head, is_saw, ref_user_id) 
        VALUES ('{data_uniq_id}', '{user_id}', '{utc_time}', '{notification}', 'Income Created', 0, '{user_id}')
        """
    ...
    #To modify the data
        ...
        query = f"""update income set income_type = '{income_type}', income_type_id = '{income_type_id}',cash_type='{cash_type}',remarks='{remarks}', amount='{amount}',income_date='{income_date}', modified_date = '{utc_time}',modified_by = '{user_id}' where data_uniq_id = '{data_uniq_id}';"""
    ...
    #To delete the data
        ...
        data_uniq_id = base64_operation(data["data_uniq_id"],'decode')  
        query = """DELETE FROM income where data_uniq_id = '{data_uniq_id}';""".format(data_uniq_id=data_uniq_id)
...
def income_delete(request):
    ...
    for data_uniq_id in data_uniq_id_list:
        ...
        query = """delete from income where data_uniq_id = '{data_uniq_id}';""".format(data_uniq_id=data_uniq_id_en)                
        execute = django_execute_query(query)
        delete_data = f"""DELETE FROM notification_table WHERE data_uniq_id='{data_uniq_id_en}';"""
        django_execute_query(delete_data)
...
def income_get(request):
    ...
    The `income_get` API is responsible for fetching data from the master database
    ...
    income_date = request.GET.get('income_date',None)
    to_income_date = request.GET.get('to_income_date',None)
    ...
    if search_input:
       ...
       search_join += """AND ({table_name}.income_type ILIKE '{inp}' OR {table_name}.remarks ILIKE '{inp}' OR {table_name}.cash_type ILIKE '{inp}' OR CAST({table_name}.amount AS TEXT) ILIKE '{inp}')""".format(inp=search_input, table_name=table_name)
    
    if cash_type:
         search_join += generate_filter_clause(f'{table_name}.cash_type',table_name,cash_type,False)

    if income_date and to_income_date:
        search_join += f" AND {table_name}.income_date BETWEEN '{income_date}' AND '{to_income_date}'"

    if income_type:
         search_join += generate_filter_clause(f'{table_name}.income_type_id',table_name,income_type,True)
    ...
    fetch_data_query = """ SELECT *, TO_CHAR(income.created_date, 'Mon DD, YYYY | HH12:MI AM') as created_f_date,(select user_name from user_master where user_master.data_uniq_id = income.created_by) as created_user FROM income WHERE 1=1  {search_join} {order_by} {limit};""".format(search_join=search_join,order_by=order_by,limit=limit_offset) 
    ...
    for index,i in enumerate(get_all_data):
        ...
        i['income_type_id'] = base64_operation(i['income_type_id'],'encode')
...
def income_filter(request):
    ...
    The `income_filter` API is responsible for adding new records to the master database.
    It expects an HTTP request object containing the data to be inserted. The data should be in a
    specific format, such as JSON, and must include the necessary fields required by the master database.
    """
    ...
    if status == 1 or status == "1" :
        ...
        search_label = f"""select * from income_filter where label = '{label}';"""
        ...
        if len(get_label) == 1:
            ...
            query = f"""insert into income_filter (data_uniq_id,label,created_date,created_by,modified_date) values ('{data_uniq_id}','{label}','{utc_time}', '{user_id}','{utc_time}')"""
            success_message = "Data created successfully"
            error_message = "Failed to create data"
            execute = django_execute_query(query)
            if execute == 0:
        ...
        query = f"""DELETE FROM income_filter WHERE label='{label}';"""
...
def income_filter_get(request):
    ...
    The `income_filter_get` API is responsible for fetching data from the master database
    based on the parameters provided in the HTTP request. The request may include filters, sorting
    criteria, or other parameters to customize the query.
    """
    
    
    table_name = 'income_filter'
    ...
    #Query to fetch all the data 
    fetch_data_query = """ SELECT *, TO_CHAR(income_filter.created_date, 'Mon DD, YYYY | HH12:MI AM') as created_f_date,(select user_name from user_master where user_master.data_uniq_id = income_filter.created_by) as created_user FROM income_filter WHERE 1=1  {search_join} {order_by} {limit};""".format(search_join=search_join,order_by=order_by,limit=limit_offset) 
